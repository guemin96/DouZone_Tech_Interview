...
데이터 베이스에서 하나의 작업을 할 때 한꺼번에 모두 수행되어야 할 작업들을 의미한다.
<br>
Commit과 Rollback을 통해 작업을 수행할 수 있다.
<br>

<h4>트랜잭션이란?</h4>
트랜잭션이란 하나의 작업으로 트랜잭션이 중단되면, 그 전에 수행한 모든 변경은 롤백이 되고(원자성) 트랜잭션 내에서 수행된 모든 조작은 DB 제약 조건을 준수해야 하고(일관성) 다른 트랜잭션의 작업이 현재 실행 중인 트랜잭션에 영향을 미치지 않아야 하며(고립성) 트랜잭션이 성공적으로 완료된 후에는 그 결과가 영구적으로 저장되어야 한다(지속성)
